...
func ApiDistributionsDelete(w http.ResponseWriter, r *http.Request) {
	req := reqio.NewRequest(w, r).PrepareRestful()
	log := req.GetContext().Get("logger").(logger.IManager)
	vm := req.GetContext().Get("view").(view.IManager)

	log.Log("distributions_delete_api_handler", "request received")
	...
	if err = distDomain.DeleteByIds(req.GetContext().Value(), payload.Ids...); err != nil {
