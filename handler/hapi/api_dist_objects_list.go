...
func ApiDistObjectsList(w http.ResponseWriter, r *http.Request) {
	req := reqio.NewRequest(w, r).PrepareRestful()
	log := req.GetContext().Get("logger").(logger.IManager)
	vm := req.GetContext().Get("view").(view.IManager)
	datasource := req.GetContext().Get("db").(database.IManager)

	log.Log("distributions_objects_list_api_handler", "request received")
	...
	var (
		items []*distribution.Object
		total int
	)
	items, total, err = distribution.NewDistributionDomain(datasource).FindAllObjects(
		req.GetContext().Value(), payload.Page, payload.Limit,
	)
	if err != nil {
