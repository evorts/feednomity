...
func ApiDistObjectsDelete(w http.ResponseWriter, r *http.Request) {
	req := reqio.NewRequest(w, r).PrepareRestful()
	log := req.GetContext().Get("logger").(logger.IManager)
	vm := req.GetContext().Get("view").(view.IManager)

	log.Log("distributions_objects_delete_api_handler", "request received")
	...
	if err = distDomain.DeleteObjectByIds(req.GetContext().Value(), payload.Ids...); err != nil {
